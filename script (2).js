...
let timerInterval=null,timerStartMs=0,timerDurationKey='90';
let timerPaused=false,timerPauseStart=0,timerPausedTotal=0;
...
function isMatchPaused(md){
  if(!md) return false;
  // If this is the actively-viewed match, the live in-memory pause state is authoritative
  if(mdId===md.id) return timerPaused;
  return !!md.htPaused;
}
...
function startTimer(md){
  stopTimer();
  if(md.status!=='live')return;
  // Use the moment the admin actually started the match, falling back to kickoff time
  timerStartMs=md.liveStartMs||kickoffMs(md)||Date.now();
  timerDurationKey=md.durationKey||'90';
  timerPaused=md.htPaused||false;
  timerPauseStart=md.htPauseStart||0;
  timerPausedTotal=md.htPausedTotal||0;
  timerInterval=setInterval(function(){updateTimerDisplay(md);},250);
  updateTimerDisplay(md);
}
function stopTimer(){if(timerInterval){clearInterval(timerInterval);timerInterval=null;}$('live-timer-bar').style.display='none';}
function updateTimerDisplay(md){
  var bar=$('live-timer-bar');
  if(!bar||md.status!=='live'){stopTimer();return;}
  bar.style.display='';
  var dur=getDuration(md);
  var totalSecs=dur.mins*60;
  var halfSecs=Math.floor(totalSecs/dur.halves);
  // Real elapsed: wall clock minus total paused time
  var nowMs=Date.now();
  var extraPause=timerPaused?(nowMs-timerPauseStart):0;
  var netElapsedMs=(nowMs-timerStartMs)-(timerPausedTotal*1000)-extraPause;
  var netElapsed=Math.max(0,Math.floor(netElapsedMs/1000));
  // Which half/quarter
  var currentHalf=Math.min(dur.halves,Math.floor(netElapsed/halfSecs)+1);
  var halfElapsed=netElapsed-((currentHalf-1)*halfSecs);
  // Stoppage time only applies when we're in the LAST half and over the cap
  var isLastHalf=currentHalf===dur.halves;
  var atCap=isLastHalf&&halfElapsed>=halfSecs;
  var displaySecs=atCap?halfSecs:Math.min(halfElapsed,halfSecs);
  var mm=Math.floor(displaySecs/60),ss=displaySecs%60;
  var timeStr=mm+':'+(ss<10?'0':'')+ss;
  if(atCap){
    var extra=halfElapsed-halfSecs;
    timeStr=Math.floor(halfSecs/60)+'+'+(extra<10?'0':'')+extra+"''";
  }
  var halfLabel;
  if(timerPaused){
    // Admin paused the clock = it's half-time (or a quarter break for Netball)
    halfLabel=dur.sport==='Netball'
      ?(currentHalf===1?'Q1/Q2 Break':currentHalf===2?'Half Time':currentHalf===3?'Q3/Q4 Break':'Break')
      :'Half Time';
  } else {
    halfLabel=dur.sport==='Netball'
      ?'Quarter '+currentHalf+' of '+dur.halves
      :(currentHalf===1?'1st Half':'2nd Half');
  }
  var pct=Math.min(100,(netElapsed/totalSecs)*100);
  // Break detection: between halves (clock hit the cap of a non-final half) and admin hasn't paused yet
  var betweenHalves=!isLastHalf&&halfElapsed>=halfSecs;
  var atBreak=!timerPaused&&betweenHalves;
  var breakLabel=dur.sport==='Netball'
    ?(currentHalf===1?'Q1/Q2 Break':currentHalf===2?'Half Time':currentHalf===3?'Q3/Q4 Break':'Break')
    :'Half Time';
  // Admin controls
  var adminCtrl='';
  if(isAdmin){
    if(timerPaused){
      ...
      adminCtrl='<button onclick="pauseTimer(\''+mdId+'\')" style="padding:5px 16px;border-radius:7px;border:1.5px solid #f39c12;color:#f39c12;background:#fff;font-size:12px;font-weight:700;cursor:pointer;margin-left:10px;animation:pulse 1.2s infinite">&#9646;&#9646; '+breakLabel+'</button>';
    } else {
  ...
  var timerColor=timerPaused?'#f39c12':'var(--c-accent)';
  var badge=timerPaused
  ...
  bar.innerHTML=
    '<div class="timer-main">'+
      '<div class="timer-display" style="color:'+timerColor+';min-width:80px">'+timeStr+'</div>'+
      '<div class="timer-info">'+
        '<div class="timer-period">'+halfLabel+'</div>'+
        '<div class="timer-quarter" style="color:#888">'+dur.label+' &middot; '+dur.sport+'</div>'+
      '</div>'+
      badge+adminCtrl+
    '</div>'+
    '<div class="timer-progress" style="margin-top:8px">'+
      '<div class="timer-progress-fill" style="width:'+pct+'%;background:'+timerColor+'"></div>'+
    '</div>'+
    pauseBanner;
}
...
function mmRenderClock(md){
  ...
  if(md.status !== 'live'){
    box.innerHTML = '<div class="mm-clock-period">Clock starts when match goes live</div>';
    ctrls.innerHTML = '';
    return;
  }
  ...
  const extraPause = timerPaused ? (nowMs - timerPauseStart) : 0;
  const netElapsedMs = (nowMs - timerStartMs) - (timerPausedTotal * 1000) - extraPause;
  const netElapsed = Math.max(0, Math.floor(netElapsedMs / 1000));
  const currentHalf = Math.min(dur.halves, Math.floor(netElapsed / halfSecs) + 1);
  const halfElapsed = netElapsed - ((currentHalf - 1) * halfSecs);
  const isLastHalf = currentHalf === dur.halves;
  const atCap = isLastHalf && halfElapsed >= halfSecs;
  const displaySecs = atCap ? halfSecs : Math.min(halfElapsed, halfSecs);
  const mm = Math.floor(displaySecs / 60), ss = displaySecs % 60;
  let timeStr = mm + ':' + (ss < 10 ? '0' : '') + ss;
  if(atCap){
    const extra = halfElapsed - halfSecs;
    timeStr = Math.floor(halfSecs/60) + '+' + (extra < 10 ? '0' : '') + extra + "''";
  }
  const halfLabel = timerPaused
    ? (dur.sport === 'Netball'
        ? (currentHalf===1?'Q1/Q2 Break':currentHalf===2?'Half Time':currentHalf===3?'Q3/Q4 Break':'Break')
        : 'Half Time')
    : (dur.sport === 'Netball'
  ...
  box.innerHTML = '<div class="mm-clock-time" style="color:' + (timerPaused ? '#f39c12' : 'var(--c-accent,#4dc8c8)') + '">' + timeStr + '</div>' +
    '<div class="mm-clock-period">' + halfLabel + (timerPaused ? ' &middot; PAUSED' : '') + '</div>';

  const betweenHalves = !isLastHalf && halfElapsed >= halfSecs;
  if(timerPaused){
    ctrls.innerHTML = '<button onclick="resumeTimer(mdId);mmRenderClock(getData(clubId).matchdays.find(m=>m.id===mdId))" style="border-color:#2ecc71;color:#2ecc71">Resume Match</button>';
  } else {
    const label = betweenHalves
      ? (dur.sport === 'Netball' ? 'Pause for Break' : 'Pause for Half-Time')
      : 'Pause Clock';
    ctrls.innerHTML = '<button onclick="pauseTimer(mdId);mmRenderClock(getData(clubId).matchdays.find(m=>m.id===mdId))" style="border-color:#f39c12;color:#f39c12">' + label + '</button>';
  }
}
...
async function mmSetStatus(status){
  ...
  if(status === 'live' && oldStatus !== 'live'){
    md.htPaused = false; md.htPausedTotal = 0; md.htPauseStart = 0;
    md.liveStartMs = Date.now(); // clock starts from this exact moment
  }
  document.querySelectorAll('.mm-status-btn').forEach(b => {
...
async function pauseTimer(mdId2){
  ...
  var d=getData(clubId),md2=d&&d.matchdays?d.matchdays.find(function(m){return m.id===mdId2;}):null;
  if(md2){
    md2.htPaused=true;md2.htPauseStart=timerPauseStart;md2.htPausedTotal=timerPausedTotal;
    sv('uc_data_v7',clubData);
    if(dbConnected){ await dbSaveMatchday(clubId,{...md2,_dbId:md2._dbId||md2.id}); }
  }
  writeLog('match_paused','matchday',{matchday_id:mdId2});
  showToast('Match Paused','Half-time break — click Resume when ready.');
  if(typeof mmRenderClock==='function'){ var __md=getData(clubId)?.matchdays?.find(function(m){return m.id===mdId2;}); if(__md) mmRenderClock(__md); }
}
async function resumeTimer(mdId2){
  if(!timerPaused)return;
  var pausedMs=Date.now()-timerPauseStart;
  timerPausedTotal+=Math.floor(pausedMs/1000);
  timerPaused=false;timerPauseStart=0;
  // Snap the 2nd half to start at exactly 0:00.
  // If the admin paused late (e.g. at 31:05 instead of 30:00), absorb
  // the overshoot into timerPausedTotal so netElapsed === halfSecs exactly.
  var d0=getData(clubId),md0=d0&&d0.matchdays?d0.matchdays.find(function(m){return m.id===mdId2;}):null;
  if(md0){
    var dur0=getDuration(md0);
    var halfSecs0=Math.floor(dur0.mins*60/dur0.halves);
    var netNow=Math.max(0,Math.floor((Date.now()-timerStartMs)/1000)-timerPausedTotal);
    // currentHalf after resume
    var halfNum=Math.min(dur0.halves,Math.floor(netNow/halfSecs0)+1);
    var halfElapsed0=netNow-((halfNum-1)*halfSecs0);
    // If we're past the boundary of a non-final half, absorb the overshoot
    if(halfNum<dur0.halves&&halfElapsed0>0){
      timerPausedTotal+=halfElapsed0;
      md0.htPausedTotal=timerPausedTotal;
    }
    md0.htPaused=false;md0.htPauseStart=0;md0.htPausedTotal=timerPausedTotal;
    sv('uc_data_v7',clubData);
    if(dbConnected){ await dbSaveMatchday(clubId,{...md0,_dbId:md0._dbId||md0.id}); }
  }
  writeLog('match_resumed','matchday',{matchday_id:mdId2});
  showToast('Match Resumed','Second half underway!');
  if(typeof mmRenderClock==='function'){ var __md=getData(clubId)?.matchdays?.find(function(m){return m.id===mdId2;}); if(__md) mmRenderClock(__md); }
}
...
function checkScheduledNotifs(){
  ...
  clubs.forEach(club=>{
    (getData(club.id)?.matchdays||[]).forEach(md=>{
      ...
      const roMs=ratingOpenMs(md);
      // Rating window opens automatically, but match status must only be set
      // live by the admin — do NOT auto-promote to live here.
...
// Ticks the small mm:ss clocks shown in the home page's "Live Now" list
// every second, without a full re-render, so seconds visibly count up.
...
function tickHomeLiveClocks(){
  ...
  document.querySelectorAll('.live-timer[data-ko]').forEach(function(el){
    var koMs=parseInt(el.getAttribute('data-ko'),10)||0;
    var mid2=el.getAttribute('data-mid');
    var cid2=el.getAttribute('data-cid');
    if(!koMs){el.textContent='LIVE';return;}
    var md2=null;
    if(cid2&&mid2){var d2=getData(cid2);if(d2)md2=(d2.matchdays||[]).find(function(m){return m.id===mid2;});}
    var dur2=md2?getDuration(md2):DURATION_MAP['90'];
    var halfSecs2=Math.floor(dur2.mins*60/dur2.halves);
    var pausedTotal=md2?(md2.htPausedTotal||0):0;
    var isPaused=md2?(!!(md2.htPaused)):false;
    var pauseStartMs=md2?(md2.htPauseStart||0):0;
    var extraPause=isPaused&&pauseStartMs?Math.floor((Date.now()-pauseStartMs)/1000):0;
    var netElapsed=Math.max(0,Math.floor((Date.now()-koMs)/1000)-pausedTotal-extraPause);
    var currentHalf=Math.min(dur2.halves,Math.floor(netElapsed/halfSecs2)+1);
    var halfElapsed=netElapsed-((currentHalf-1)*halfSecs2);
    var atCap=halfElapsed>=halfSecs2;
    var atBreak=atCap&&currentHalf<dur2.halves&&!isPaused;
    if(isPaused||atBreak){
      var breakLbl=dur2.sport==='Netball'
        ?(currentHalf===1?'Q1/Q2 Break':currentHalf===2?'Half Time':currentHalf===3?'Q3/Q4 Break':'Break')
        :'Half Time';
      el.textContent=breakLbl;
      el.style.fontStyle='italic';
      el.style.fontSize='11px';
    }else{
      el.style.fontStyle='';
      el.style.fontSize='';
      var displaySecs=atCap?halfSecs2:halfElapsed;
      var mm=Math.floor(displaySecs/60),ss=displaySecs%60;
      var timeStr;
      if(atCap){var extra=halfElapsed-halfSecs2;timeStr=Math.floor(halfSecs2/60)+'+'+(extra<10?'0':'')+extra+"'";}
      else{timeStr=mm+':'+(ss<10?'0':'')+ss;}
      var halfLbl=dur2.sport==='Netball'?'Q'+currentHalf:(currentHalf===1?'1st':'2nd');
      el.textContent=halfLbl+' '+timeStr;
    }
  });
}
...
function goBack(){
  if(mdId){
    ...
    var tp=document.getElementById('tab-matchdays');if(tp)tp.classList.add('active');
...
function renderHome(){
  ...
  if(liveMds.length){
    ...
    liveList.innerHTML=liveMds.map(function(item){
      var club=item.club,md=item.md;
      var koMs=kickoffMs(md);
      var dur=getDuration(md);
      var halfSecs=Math.floor(dur.mins*60/dur.halves);
      var pausedTotal=md.htPausedTotal||0;
      var isPaused=!!(md.htPaused);
      var pauseStartMs=md.htPauseStart||0;
      var extraPause=isPaused&&pauseStartMs?Math.floor((Date.now()-pauseStartMs)/1000):0;
      var netElapsed=koMs?Math.max(0,Math.floor((Date.now()-koMs)/1000)-pausedTotal-extraPause):0;
      var currentHalf=Math.min(dur.halves,Math.floor(netElapsed/halfSecs)+1);
      var halfElapsed=netElapsed-((currentHalf-1)*halfSecs);
      var atCap=halfElapsed>=halfSecs;
      var atBreak=atCap&&currentHalf<dur.halves&&!isPaused;
      var timerInner;
      if(!koMs){
        timerInner='LIVE';
      }else if(isPaused||atBreak){
        timerInner=dur.sport==='Netball'
          ?(currentHalf===1?'Q1/Q2 Break':currentHalf===2?'Half Time':currentHalf===3?'Q3/Q4 Break':'Break')
          :'Half Time';
      }else{
        var displaySecs=atCap?halfSecs:halfElapsed;
        var mm=Math.floor(displaySecs/60),ss=displaySecs%60;
        var tStr;
        if(atCap){var extra=halfElapsed-halfSecs;tStr=Math.floor(halfSecs/60)+'+'+(extra<10?'0':'')+extra+"'";}
        else{tStr=mm+':'+(ss<10?'0':'')+ss;}
        var halfLbl=dur.sport==='Netball'?'Q'+currentHalf:(currentHalf===1?'1st Half':'2nd Half');
        timerInner=halfLbl+' '+tStr;
      }
      var isHT=koMs&&(isPaused||atBreak);
      var timerStyle=isHT?'font-style:italic;font-size:11px':'';
      var scorersH=liveGoalScorersH(club,md,'live-row-scorers');
      return '<div class="live-match-row" style="cursor:pointer" onclick="goToLiveMatch(\''+club.id+'\',\''+md.id+'\')">'+
        logoH(club,36)+
        '<div class="live-timer" data-ko="'+(koMs||0)+'" data-mid="'+md.id+'" data-cid="'+club.id+'" style="'+timerStyle+'">'+timerInner+'</div>'+
        '<div style="flex:1;min-width:0"><div style="font-weight:700;font-size:14px;color:#1a1a2e">'+club.short+' vs '+md.opponent+'</div>'+(md.venue?'<div style="font-size:11px;color:#999">&#128205; '+md.venue+'</div>':'')+scorersH+'</div>'+
...
function clubCardH(club){
  ...
  return`<div class="club-card">
    ...
    ${liveMds.length?`<div class="cc-live-row">${liveMds.map(m=>`<div class="cc-live-item"><span class="live-badge${isMatchPaused(m)?' paused-badge':''}" style="cursor:pointer" onclick="goToLiveMatch('${club.id}','${m.id}')">${isMatchPaused(m)?'<span class="pause-icon">&#10074;&#10074;</span>'+pausedBreakLabel(m):'<span class="live-dot"></span>LIVE'} vs ${m.opponent} &middot; ${m.homeGoals||0}-${m.awayGoals||0}</span>${liveGoalScorersH(club,m,'cc-live-scorers')}</div>`).join('')}</div>`:''}
...
async function doAddMd(){
  ...
  if(!lbl||!opp)return;
  const newMd={id:'md'+Date.now(),label:lbl,opponent:opp,venue,date,kickoffTime:time,result:res,status:'upcoming',homeGoals:0,awayGoals:0,ratingWindowHrs:24,forceClose:false,ratingOpenOverride:null,durationKey:dur};
  if(dbConnected){
    var dbMd=await dbSaveMatchday(clubId,newMd);
...
async function doSaveMdEdit(){
  ...
  if(md.forceClose)md.status='finished';
  if(oldStatus!=='live'&&md.status==='live'&&!md.liveStartMs){
    md.liveStartMs=Date.now(); // clock starts from when admin goes live
  }
  ...
  if(mdId===editMdId)renderMd();else{renderMatchdays();updateLiveIndicator();}
}
