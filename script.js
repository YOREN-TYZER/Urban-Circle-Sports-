...
async function dbSaveMatchday(cid, md) {
  try {
    ...
    const rowId = md._dbId || md.id;
    if (rowId) {
      await sb('PATCH', 'matchdays', {eq: {id: rowId}, data});
      if (!md._dbId) md._dbId = rowId;
    } else {
...
function startTimer(md){
  stopTimer();
  const koMs=kickoffMs(md);
  if(!koMs||md.status!=='live')return;
  timerStartMs=koMs;
...
function updateTimerDisplay(md){
  ...
  // Cumulative seconds from match start for this half
  var halfStartSecs=(currentHalf-1)*halfSecs;
  // At cap of this half → show stoppage time
  var atCap=halfElapsed>=halfSecs;
  // Display cumulative match minutes (e.g. 2nd half shows 45:01, 45:02...)
  var displaySecs=atCap?halfSecs:halfElapsed;
  var cumulativeSecs=halfStartSecs+displaySecs;
  var mm=Math.floor(cumulativeSecs/60),ss=cumulativeSecs%60;
  var timeStr=mm+':'+(ss<10?'0':'')+ss;
  if(atCap){
    var extra=halfElapsed-halfSecs;
    timeStr=Math.floor((halfStartSecs+halfSecs)/60)+'+'+(extra<10?'0':'')+extra+"''";
  }
  var halfLabel;
  if(timerPaused){
  ...
  // Break detection: first half just ended and not yet paused
  var atBreak=!timerPaused&&atCap&&currentHalf<dur.halves;
...
function mmRenderClock(md){
  ...
  const halfStartSecs = (currentHalf - 1) * halfSecs;
  const atCap = halfElapsed >= halfSecs;
  const displaySecs = atCap ? halfSecs : halfElapsed;
  const cumulativeSecs = halfStartSecs + displaySecs;
  const mm = Math.floor(cumulativeSecs / 60), ss = cumulativeSecs % 60;
  ...
  if(timerPaused){
    ...
    const label = atCap && currentHalf < dur.halves
...
async function mmSetStatus(status){
  ...
  if(status === 'live' && oldStatus !== 'live'){
    md.htPaused = false; md.htPausedTotal = 0;
  }
  document.querySelectorAll('.mm-status-btn').forEach(b => {
...
async function resumeTimer(mdId2){
  ...
  var d=getData(clubId),md2=d&&d.matchdays?d.matchdays.find(function(m){return m.id===mdId2;}):null;
  if(md2){
    md2.htPaused=false;md2.htPauseStart=0;md2.htPausedTotal=timerPausedTotal;
    sv('uc_data_v7',clubData);
    if(dbConnected){ await dbSaveMatchday(clubId,{...md2,_dbId:md2._dbId||md2.id}); }
  }
...
function checkScheduledNotifs(){
  ...
  clubs.forEach(club=>{
    (getData(club.id)?.matchdays||[]).forEach(md=>{
      ...
      if(roMs&&now>=roMs&&md.status==='upcoming'&&!md.forceClose){
        md.status='live';sv('uc_data_v7',clubData);
        if(dbConnected){ dbSaveMatchday(club.id,{...md,_dbId:md._dbId||md.id}); }
        const liveKey=club.id+'_'+md.id+'_live';
        if(!notifSent[liveKey]){notifSent[liveKey]=true;sv('uc_notifs_v7',notifSent);sendNotif('Match is LIVE',`${club.short} vs ${md.opponent} - rate now!`);}
        if(clubId===club.id&&activeTab==='matchdays')renderMatchdays();
        if(clubId===club.id&&mdId===md.id)renderMd();
...
function tickHomeLiveClocks(){
  ...
  document.querySelectorAll('.live-timer[data-ko]').forEach(function(el){
    ...
    if(isPaused||atBreak){
      ...
      el.style.fontStyle='';
      el.style.fontSize='';
      var halfStartSecs2=(currentHalf-1)*halfSecs2;
      var displaySecs=atCap?halfSecs2:halfElapsed;
      var cumulativeSecs2=halfStartSecs2+displaySecs;
      var mm=Math.floor(cumulativeSecs2/60),ss=cumulativeSecs2%60;
      var timeStr;
      if(atCap){var extra=halfElapsed-halfSecs2;timeStr=Math.floor((halfStartSecs2+halfSecs2)/60)+'+'+(extra<10?'0':'')+extra+"'";}
