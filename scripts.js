const SUPA_URL = 'https://oqbzvjkzsapqrqqikkcr.supabase.co';
const SUPA_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9xYnp2amt6c2FwcXJxcWlra2NyIiwicm9sZSI6ImFub24iLCJpYXQiOjE3ODU2MDc4NzcsImV4cCI6MjEwMTE4Mzg3N30.4Xm_APTw7_rGvC8k77oaZf_vykll8h5dzDdio21HlNg';
...
// Every football formation reserves the "0_0" slot for the Goalkeeper
// (see FORMATIONS above — row 0 is always ['GK']), so a clean sheet for a
// given matchday is: this player was in that slot, the match has actually
// finished (not still live/upcoming, since it could still concede), and
// the opponent's goal count for that match was zero.
function getPlayerCleanSheets(cid, pid){
  if(isNetball(cid)) return 0;
  let count=0;
  const data=getData(cid);
  Object.keys(lineups).forEach(k=>{
    if(k.indexOf(cid+'_')!==0) return;
    const lu=lineups[k];if(!lu||!lu.slots) return;
    if(lu.slots['0_0']!==pid) return;
    const mid=k.slice((cid+'_').length);
    const md=data?.matchdays?.find(m=>String(m.id)===String(mid));
    if(!md||md.status!=='finished') return;
    if((md.awayGoals||0)===0) count++;
  });
  return count;
}
...
function effectiveCS(p, cid){
  if(p.csOverride!=null) return p.csOverride;
  return cid ? getPlayerCleanSheets(cid, p.id) : (p.cleanSheets||0);
}
...
function computeStatValue(p, f, cid){
  ...
  if(f.key === 'cleanSheets'){
    return effectiveCS(p, cid);
  }
  if(f.key === 'cleanSheetPct'){
    const gp = effectiveGP(p, cid);
    const cs = effectiveCS(p, cid);
...
function renderHubStats(){
  ...
  targetClubs.forEach(function(club){
    ...
    groups.forEach(function(sg){
      var valFor=function(p){ return sg.key==='cleanSheets' ? effectiveCS(p,club.id) : (p[sg.key]||0); };
      var list=players.filter(function(p){return valFor(p)>0;});
      if(!list.length)return;
      list.sort(function(a,b){return valFor(b)-valFor(a);});
      ...
      list.slice(0,5).forEach(function(p,i){
        ...
        clubHtml+='<div class="stat-rank-val" style="color:'+sg.color+'">'+valFor(p)+'</div>';
        clubHtml+='</div>';
      });
      clubHtml+='</div>';
    });
