...
async function sb(method, table, opts={}) {
  const {eq, data, select, order, limit, upsert, neq} = opts;
...
async function dbSaveMatchday(cid, md) {
  try {
    const data = {
      ...
      ht_paused: md.htPaused||false, ht_paused_total: md.htPausedTotal||0
    };
    if (md._dbId) {
...
function updateTimerDisplay(md){
  ...
  var halfLabel=dur.sport==='Netball'
    ?'Quarter '+currentHalf+' of '+dur.halves
    :'Half '+currentHalf+' of '+dur.halves;
...
function mmRenderClock(md){
  ...
  const halfLabel = dur.sport === 'Netball'
    ? 'Quarter ' + currentHalf + ' of ' + dur.halves
    : 'Half ' + currentHalf + ' of ' + dur.halves;
...
function checkScheduledNotifs(){
  ...
  clubs.forEach(club=>{
    (getData(club.id)?.matchdays||[]).forEach(md=>{
      ...
      const oneHrKey=club.id+'_'+md.id+'_1hr';
      if(!notifSent[oneHrKey]&&ko-now>0&&ko-now<=3600000){notifSent[oneHrKey]=true;sv('uc_notifs_v7',notifSent);sendNotif('Match in 1 Hour',`${club.short} vs ${md.opponent} kicks off soon!`);}
...
setInterval(checkScheduledNotifs,30000);
...
function renderHome(){
  ...
  if(liveMds.length){
    ...
    liveList.innerHTML=liveMds.map(({club,md})=>{
      const koMs=kickoffMs(md),elapsed=koMs?Math.floor((Date.now()-koMs)/1000):0;
      const mSec=Math.floor(elapsed/60),sSec=elapsed%60;
      const timeStr=koMs?`${mSec}:${sSec.toString().padStart(2,'0')}`:'LIVE';
      return`<div class="live-match-row" onclick="enterClub('${club.id}');setTimeout(()=>enterMd('${md.id}'),50)">
        ${logoH(club,36)}
        <div class="live-timer">${timeStr}</div>
        <div style="flex:1"><div style="font-weight:700;font-size:14px;color:#1a1a2e">${club.short} vs ${md.opponent}</div>${md.venue?`<div style="font-size:11px;color:#999">&#128205; ${md.venue}</div>`:''}</div>
...
function clubCardH(club){
  ...
  return`<div class="club-card">
    ...
    ${liveMds.length?`<div class="cc-live-row">${liveMds.map(m=>`<span class="live-badge${isMatchPaused(m)?' paused-badge':''}" style="cursor:pointer" onclick="enterClub('${club.id}');setTimeout(()=>enterMd('${m.id}'),50)">${isMatchPaused(m)?'<span class="pause-icon">&#10074;&#10074;</span>'+pausedBreakLabel(m):'<span class="live-dot"></span>LIVE'} vs ${m.opponent}</span>`).join(' ')}</div>`:''}
